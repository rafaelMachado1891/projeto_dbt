...
class S3Client:
    def __init__(self):
        ...
        for var in ("aws_access_key_id", "aws_secret_access_key", "s3_bucket"):
            if self._envs[var] is None:
                print(f"A variavel de ambiente {var} nao esta definida.")
                sys.exit(1)
    ...
    def upload_file(self, data, s3_key):
        try:
            ...
            print(
                "Credenciais nao encontradas. Configure as credenciais AWS corretamente."
            )
    ...
    def download_file(self, s3_key):
        try:
            ...
            print(f"Arquivo {s3_key} nao encontrado no bucket {self._envs['s3_bucket']}.")
